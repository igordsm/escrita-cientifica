...
PANDOC_HANDOUT = 'pandoc -f markdown+backtick_code_blocks+fenced_divs+raw_html+grid_tables --lua-filter=filters/message.lua --lua-filter=filters/spacer.lua --lua-filter=filters/graphviz.lua --lua-filter=filters/vtable.lua  -s --template templates/tufte-handout.tex '
PANDOC_PAGE = 'pandoc -f gfm+attributes --toc -s --template templates/template-index.html '
